...
// Puts the given name and has its declaration at the given file location (floc),
...
// Builds the symbol table and checks the declarations in cds.
...
// Checks the const declaration and adds it to the current 
// scope's symbol table. Error produced if the name has already been declared.
...
// Checks the statement to make sure that
// all identifiers referenced in it have
// been declared (if not, then produce an error).
...
// Checks assignment statement for accuracy,
// if not accurate then produce an error.
...
// Checks while statement for accuracy,
// if not accurate then produce an error.
...
// Checks begin statement for accuracy,
// if not accurate then produce an error.
...
// Checks binary condition for accuracy,
// if not accurate then produce an error.
...
// Checks odd condition for accuracy,
// if not accurate then produce an error
...
// Checks if statement for accuracy,
// if not accurate then produce an error.
...
// Checks read statement for accuracy,
// if not accurate then produce an error.
...
// Checks write statement for accuracy,
// if not accurate then produce an error.
...
// Checks that the given name has been declared,
// if not, then produce an error via the file location given.
...
// Checks the expression (exp) for accuracy,
// if not accurate then produce an error.
